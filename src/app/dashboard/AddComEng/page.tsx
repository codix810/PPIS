...
export default function AddComEngPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('folder', 'ppis-ComEng');
      ...
      const ComEngData = { title, content, imageUrl };

      const res = await fetch('/api/ComEng', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(ComEngData),
      });

      if (!res.ok) throw new Error('Failed to add ComEng');

      showMessage('success', 'ComEng added successfully!');
  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-white rounded-lg shadow mt-10">
      <h1 className="text-2xl font-bold mb-6 text-center">Add ComEng</h1>
