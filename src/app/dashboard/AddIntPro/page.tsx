...
export default function AddIntProPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('folder', 'ppis-IntPro');
      ...
      const IntProData = { title, content, imageUrl };

      const res = await fetch('/api/IntPro', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(IntProData),
      });

      if (!res.ok) throw new Error('Failed to add IntPro');

      showMessage('success', 'IntPro added successfully!');
  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-white rounded-lg shadow mt-10">
      <h1 className="text-2xl font-bold mb-6 text-center">Add IntPro</h1>
